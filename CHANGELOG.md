...
### Fixed
- Галерея вложений: закрытие вертикальным свайпом не работало на Android/Fabric —
  горизонтальный `FlatList` перехватывал касание, а bubble-фаза
  `onMoveShouldSetResponder` внутри `Modal` не опрашивалась. Галерея переведена на
  единый `PanResponder` (пейджинг + закрытие) с захватом жеста в capture-фазе;
  нативного `ScrollView` больше нет. `translateX` ведёт листание, `translateY` —
  закрытие, ось фиксируется в начале жеста. Все анимации на `useNativeDriver: false`
  (JS-драйвер питает интерполяции прозрачности фона и хрома).
- Стрелки `‹ ›` снова листают вложения по клику. Capture-захват жеста перехватывал
  тап по дочерним кнопкам, поэтому стрелки вынесены из контейнера с `panHandlers`
  (рендерятся соседями ленты, абсолютным позиционированием по центру по вертикали,
  с отступом от хедера и футера) — клик по ним и горизонтальный свайп ведут к одному
  и тому же пейджингу.

## [0.1.0-beta.15]

### Fixed
- Свайп-закрытие галереи вложений не срабатывало: жест-обработчик висел на
  родителе горизонтального `FlatList`, а нативный скролл забирал касание первым
  (фаза всплытия `onMoveShouldSetPanResponder` до родителя уже не доходила).
  Добавлен перехват в фазе capture (`onMoveShouldSetPanResponderCapture`) с тем
  же условием доминирования вертикали — теперь вертикальный свайп закрывает
  галерею, а горизонтальное листание картинок продолжает работать.
